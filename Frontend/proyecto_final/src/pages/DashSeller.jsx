...
export const DashSeller = () => {
  ...
  const { response, error } = ApiHook("get", `${url}?rol=vendedor`);
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(url, {
          params: { page: 1, limit: 5, deleted: false, rol: "vendedor" },
        });

        const vendedores = response.data.data.filter(
          (user) => user.rol === "vendedor"
        );

        setUserData(vendedores);
      } catch (error) {
  ...
  const handleGeneratePDF = (userId = null) => {
    try {
      ...
      const selectedSeller = userData.find((user) => user._id === userId);

      const filename = userId
        ? `vendedor_${selectedSeller.nombre}_${selectedSeller.apellido}.pdf`
        : "vendedor.pdf";
      ...
      if (selectedSeller) {
        ...
        const data = [
          [
            ...
            selectedSeller.apellido,
            selectedSeller.correo,
            selectedSeller.cedula,
            selectedSeller.nro_tlf,
            selectedSeller.fechaCreacion,
          ],
        ];
  ...
  return (
    <>
      ...
      {showModalSl && (
        <ModalCrSl
          show={showModalSl}
          setShowModalCreateSl={setShowModalSl}
          Api={url}
        />
      )}
