...
export const DashSales = () => {
  ...
  const { response, error } = ApiHook("get", "http://localhost:3001/ventas");

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:3001/ventas", {
          params: { page: 1, limit: 5, deleted: false },
        });
  ...
  const handleGeneratePDF = (userId = null) => {
    try {
      ...
      const selectedSale = sales.find((user) => user._id === userId);

      const filename = userId
        ? `venta_${selectedSale.numeroFactura}.pdf`
        : "venta.pdf";
      ...
      if (selectedSale) {
        ...
        const data = [
          [
            selectedSale.numeroFactura,
            selectedSale.cliente,
            selectedSale.vendedor,
            selectedSale.productosDetalles,
            selectedSale.productosPrecioUnitario,
            selectedSale.productosCantidadProductos,
            selectedSale.productoSubTotal,
            selectedSale.productoTotal,
          ],
        ];
        ...
        >
          ...
          <tbody>
            {searchResults.length > 0
              ? searchResults.map((sales, index) => (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td>{sales.cliente}</td>
                    <td>{sales.vendedor}</td>
                    <td>{sales.producto}</td>
                    <td>{sales.numeroFactura}</td>
                    <td>{sales.precioUnitario}</td>
                    ...
                    <td>
                      <div className="flex flex-row">
                        <Button
                          ...
                          onClick={() => handleEditClick(sales._id)}
                        >
                          <FaRegEdit />
                        </Button>
                        <Button
                          className="btn btn-warning"
                          onClick={() => handleGeneratePDF(sales._id)}
                        >
                          <IoPrintOutline />
                        </Button>
                        <Button
              ...
              : sales.map((sales, index) => (
                  <tr key={index}>
                    ...
                    <td>{sales.cliente}</td>
                    <td>{sales.vendedor}</td>
                    <td>{sales.productosDetalles}</td>
                    <td>{sales.numeroFactura}</td>
                    <td>{sales.productosPrecioUnitario}</td>
                    <td>{sales.productosCantidadProductos}</td>
                    <td>{sales.productoSubTotal}</td>
                    <td>{sales.productoTotal}</td>
                    <td>
                      <div className="flex flex-row">
                        <Button
                          className="btn btn-primary"
