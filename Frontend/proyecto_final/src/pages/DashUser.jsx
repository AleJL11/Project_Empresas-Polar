...
export const DashUser = () => {
  ...
  const [showModalCr, setShowModalCr] = useState(false);
  ...
  const { response, error } = ApiHook("get", url);
  ...
  const handleGeneratePDF = (userId = null) => {
    try {
      ...
      const filename = userId
        ? `usuario_${selectedUser.nombre}_${selectedUser.apellido}.pdf`
        : "usuario.pdf";
  ...
  return (
    <>
      ...
      {showModalCr && (
        <ModalCrUs
          show={showModalCr}
          setShowModalCreatePr={setShowModalCr}
          Api={url}
        />
      )}
      ...
      >
        ...
        >
          ...
          mr-4"
          >
            <FaArrowLeft className="lg:block min-[320px]:hidden lg:text-white" onClick={handleGoBack} />
...
<div
      ...
      <div className="w-full h-auto mx-auto text-center">
        <button className="lg:w-40 lg:h-16 lg:text-[25px] rounded-xl bg-blue-500 text-white" onClick={() => openCreate()}>Crear Usuario</button>
      </div>
